...
function generateCategoriesSection(
  ...
  categories.forEach((category, index) => {
    ...
    const catTitle = queries.categories.filter((_, index) => index % 2 === 0)
    const catParagraph = queries.categories.filter((_, index) => index % 2 != 0)
      ...
      <tr>
        <td style="background-color: ${category?.background || background}; color: ${category?.color || "#000000"}">
          ${Category({
            ...
            last: category.last,
            extraCtaLink: getCategoryLink('https://www.beliani.ch/content/creating-wall-gallery/'),
            extraCta: queries.extraCta,
            idx: index,
            len: categories.length - 1, // Ostatni element
            align: category.align, // Ustawione na "left" zamiast "center"
            line:
...
export async function artificalPlants({
  links,
  getProductById,
  getCategoryLink,
  getCategoryTitle,
                      ...
                      <tbody>
                        <tr>
                          <td style="padding-top: 0px; padding-left: 0px; padding-right: 0px; text-align: center;">
                                ...
                                <a href="${getCategoryLink('https://www.beliani.ch/accessories-decor/artificial-plants-and-flowers/')}" style="color:#000; text-decoration: underline;">
                                  <span class="newsletterCta">${queries.CTA}</span>
                                </a>
